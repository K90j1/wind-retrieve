...
module SwaggerClient
  class CustomExchangeRatesApi
    ...
    # @param start_date Start date of the date range for which you want to get exchange rates.  The date must be in yyyy-mm-dd format, for example, 2016-01-15. The start date cannot be later than the end date. 
    # @param end_date End date of the date range for which you want to get exchange rates.  The date must be in yyyy-mm-dd format, for example, 2016-01-16. The end date can be a maximum of 90 days after the start date. 
    # @param [Hash] opts the optional parameters
    # @return [GETCustomExchangeRatesType]
    def g_et_custom_exchange_rates(currency, start_date, end_date, opts = {})
      data, _status_code, _headers = g_et_custom_exchange_rates_with_http_info(currency, start_date, end_date, opts)
      return data
    end
    ...
    # @param end_date End date of the date range for which you want to get exchange rates.  The date must be in yyyy-mm-dd format, for example, 2016-01-16. The end date can be a maximum of 90 days after the start date. 
    # @param [Hash] opts the optional parameters
    # @return [Array<(GETCustomExchangeRatesType, Fixnum, Hash)>] GETCustomExchangeRatesType data, response status code and response headers
    def g_et_custom_exchange_rates_with_http_info(currency, start_date, end_date, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: CustomExchangeRatesApi.g_et_custom_exchange_rates ..."
      end
      # verify the required parameter 'currency' is set
      fail ArgumentError, "Missing the required parameter 'currency' when calling CustomExchangeRatesApi.g_et_custom_exchange_rates" if currency.nil?
      # verify the required parameter 'start_date' is set
      fail ArgumentError, "Missing the required parameter 'start_date' when calling CustomExchangeRatesApi.g_et_custom_exchange_rates" if start_date.nil?
      # verify the required parameter 'end_date' is set
      fail ArgumentError, "Missing the required parameter 'end_date' when calling CustomExchangeRatesApi.g_et_custom_exchange_rates" if end_date.nil?
      # resource path
      local_var_path = "/custom-exchange-rates/{currency}".sub('{format}','json').sub('{' + 'currency' + '}', currency.to_s)
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'GETCustomExchangeRatesType')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: CustomExchangeRatesApi#g_et_custom_exchange_rates\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
