...
module SwaggerClient
  class HostedPagesApi
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [String] :version_number Version of the Payment Pages for which you want to retrieve the configuration information. Specify 1 for Payment Pages 1.0 or 2 for Payment Pages 2.0. If omitted, information for all versions of Payment Pages are returned.  The response also depends on your tenant settings for Payment Pages 1.0 and Payment Pages 2.0. For example, if only the tenant setting for Payment Pages 2.0 is enabled, the response will only contain information for Payment Pages 2.0 forms even when this parameter is omitted. 
    # @return [Array<(GetHostedPagesType, Fixnum, Hash)>] GetHostedPagesType data, response status code and response headers
    def get_hosted_pages_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: HostedPagesApi.get_hosted_pages ..."
      end
      # resource path
      local_var_path = "/hostedpages".sub('{format}','json')

      # query parameters
      query_params = {}
      query_params[:'versionNumber'] = opts[:'version_number'] if !opts[:'version_number'].nil?
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'GetHostedPagesType')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: HostedPagesApi#get_hosted_pages\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
